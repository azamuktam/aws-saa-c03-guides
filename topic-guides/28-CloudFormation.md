...
You describe your AWS infrastructure — VPCs, EC2, RDS, IAM roles, and other resources — in a **YAML or JSON template**, and CloudFormation creates and manages those resources for you. A running instance of a template is called a **stack**.

This allows you to:

* Store infrastructure definitions in version control
* Deploy identical environments repeatedly
* Deploy infrastructure across Regions and accounts
* Update infrastructure consistently
* Delete resources together as a stack

If stack creation fails, CloudFormation **rolls back automatically by default**.
...
* **CreationPolicy + `cfn-signal`** — make CloudFormation **wait for resource initialization/configuration to finish** before considering creation successful. "EC2 must finish installing/configuring applications before stack creation proceeds" → CreationPolicy + `cfn-signal`.
...
**Elastic Beanstalk contrast:** Beanstalk is web-app-shaped automation — hand it code, it builds the standard web stack (and uses CloudFormation underneath). CloudFormation itself can build **any** infrastructure in any shape.
...
> *"Reuse a common VPC pattern across many templates"* → **Nested Stacks** (componentize the template)

> *"Wait until EC2 finishes installing and configuring applications before stack creation continues"* → **CreationPolicy + `cfn-signal`**
...
| Wait for EC2 initialization           | CreationPolicy + `cfn-signal`    |
| Failed creation                       | Automatic rollback (default)     |

You now know how to define, update, protect, and deploy infrastructure with CloudFormation.
